...
See the discussion under the mounttab type for usage."
  ...
  newproperty(:device) do
    desc "The device providing the mount.  This can be whatever device is supporting by the mount, including network devices or devices specified by UUID rather than device path, depending on the operating system.  If you already have an entry in your fstab (or you use the mounttab type to create such an entry), it is generally not necessary to specify the fstype explicitly"
  ...
  newproperty(:fstype) do
    desc "The mount type.  Valid values depend on the operating system.  If you already have an entry in your fstab (or you use the mounttab type to create such an entry), it is generally not necessary to specify the fstype explicitly"
  ...
  newproperty(:node, :requires_features => :aix) do
    ...
    validate do |value|
      ...
      raise Puppet::Error, "node is not allowed to contain slashes" if value.include?('/') or value.include?('\\')
      raise Puppet::Error, "node is not allowed to contain at signs" if value.include?('@')
    end
  end
  ...
  newparam(:name, :namevar => true) do
    ...
    validate do |value|
      ...
      raise Puppet::Error, "name is not allowed to have trailing slashes" if value =~ %r{/$} and value != '/'
      raise Puppet::Error, "name must be an absolute path" if value =~ %r{^[^/]} or value.include?('..')
    end
  end
